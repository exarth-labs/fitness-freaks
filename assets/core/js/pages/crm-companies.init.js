var checkAll = document.getElementById("checkAll"), perPage = (checkAll && (checkAll.onclick = function () {
    for (var e = document.querySelectorAll('.form-check-all input[type="checkbox"]'), t = document.querySelectorAll('.form-check-all input[type="checkbox"]:checked').length, i = 0; i < e.length; i++) e[i].checked = this.checked, e[i].checked ? e[i].closest("tr").classList.add("table-active") : e[i].closest("tr").classList.remove("table-active");
    document.getElementById("remove-actions").style.display = 0 < t ? "none" : "block"
}), 8), editlist = !1, options = {
    valueNames: ["id", "name", "owner", "industry_type", "star_value", "location", "employee", "website", "contact_email", "since", {
        attr: "src",
        name: "image_src"
    ...
    var t = 1 == e.i, i = e.i > e.matchingItems.length - e.page;
...
xhttp.onload = function () {
    ...
    Array.from(e).forEach(function (e) {
        ...
        }), companyList.sort("id", {order: "desc"}), refreshCallbacks()
    }), companyList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ001</a>')
}, xhttp.open("GET", "assets/json/company-list.json"), xhttp.send(), isCount = (new DOMParser).parseFromString(companyList.items.slice(-1)[0]._values.id, "text/html"), document.querySelector("#company-logo-input").addEventListener("change", function () {
    var e = document.querySelector("#companylogo-img"), t = document.querySelector("#company-logo-input").files[0],
        i = new FileReader;
    i.addEventListener("load", function () {
        e.src = i.result
    }, !1), t && i.readAsDataURL(t)
});
var isValue = isCount.body.firstElementChild.innerHTML, idField = document.getElementById("id-field"),
    companyNameField = document.getElementById("companyname-field"),
    companyLogoImg = document.getElementById("companylogo-img"), ownerField = document.getElementById("owner-field"),
    ...
    employeeField = document.getElementById("employee-field"), websiteField = document.getElementById("website-field"),
    contact_emailField = document.getElementById("contact_email-field"),
    sinceField = document.getElementById("since-field"), addBtn = document.getElementById("add-btn"),
    editBtn = document.getElementById("edit-btn"), removeBtns = document.getElementsByClassName("remove-item-btn"),
    editBtns = document.getElementsByClassName("edit-item-btn"),
    table = (viewBtns = document.getElementsByClassName("view-item-btn"), refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function (e) {
        e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Company", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Company", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Company") : (document.getElementById("exampleModalLabel").innerHTML = "List Company", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
        ...
        ischeckboxcheck()
    }), document.getElementById("customerTable")), tr = table.getElementsByTagName("tr"),
    trlist = table.querySelectorAll(".list tr"), count = 11, forms = document.querySelectorAll(".tablelist-form");

function ischeckboxcheck() {
    Array.from(document.getElementsByName("chk_child")).forEach(function (i) {
        i.addEventListener("change", function (e) {
            1 == i.checked ? e.target.closest("tr").classList.add("table-active") : e.target.closest("tr").classList.remove("table-active");
    ...
    removeBtns && Array.from(removeBtns).forEach(function (e) {
        e.addEventListener("click", function (e) {
            ...
            Array.from(e).forEach(function (e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function () {
                    companyList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click()
                })
            })
        })
        ...
        e.addEventListener("click", function (e) {
            ...
            Array.from(e).forEach(function (e) {
                ...
                t == itemId && (editlist = !0, idField.value = t, companyLogoImg.src = e._values.image_src, companyNameField.value = e._values.name, ownerField.value = e._values.owner, industry_typeField.value = e._values.industry_type, star_valueField.value = e._values.star_value, locationField.value = e._values.location, employeeField.value = e._values.employee, websiteField.value = e._values.website, contact_emailField.value = e._values.contact_email, sinceField.value = e._values.since)
            })
        })
        ...
        e.addEventListener("click", function (e) {
            ...
            e = companyList.get({id: itemId});
            Array.from(e).forEach(function (e) {
                (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && (e = `
                        <div class="card-body text-center">
                            <div class="position-relative d-inline-block">
                                <div class="avatar-md">
                                    <div class="avatar-title bg-light rounded-circle">
                                        <img src="${e._values.image_src}" alt="" class="avatar-sm rounded-circle object-fit-cover">
                                    </div>
                                </div>
                            </div>
...
function clearFields() {
    companyLogoImg.src = "assets/images/users/multi-user.jpg", companyNameField.value = "", ownerField.value = "", industry_typeField.value = "", star_valueField.value = "", locationField.value = "", employeeField.value = "", websiteField.value = "", contact_emailField.value = "", sinceField.value = ""
}
...
function deleteMultiple() {
    ...
    "undefined" != typeof ids_array && 0 < ids_array.length ? Swal.fire({
        ...
        if (e.value) {
            for (i = 0; i < ids_array.length; i++) companyList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${ids_array[i]}</a>`);
...
Array.prototype.slice.call(forms).forEach(function (i) {
    i.addEventListener("submit", function (e) {
        var t;
        i.checkValidity() ? (e.preventDefault(), "" === companyNameField.value || "" === ownerField.value || "" === industry_typeField.value || "" === star_valueField.value || "" === locationField.value || "" === employeeField.value || "" === websiteField.value || "" === contact_emailField.value || "" === sinceField.value || editlist ? "" !== companyNameField.value && "" !== ownerField.value && "" !== industry_typeField.value && "" !== star_valueField.value && "" !== locationField.value && "" !== employeeField.value && "" !== websiteField.value && "" !== contact_emailField.value && "" !== sinceField.value && editlist && (t = companyList.get({id: idField.value}), Array.from(t).forEach(function (e) {
            ...
            contact_email: contact_emailField.value,
            since: sinceField.value
        }), companyList.sort("id", {order: "desc"}), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), count++, Swal.fire({
            position: "center",
            icon: "success",
